       ...
       MPI_Bcast(&len, 1, MPI_INT, 0, MPI_COMM_WORLD);
		...
		//for (i=0; i<1000; ++i)
		  //  printf("[%d]: %c", i, encrypted[i]);
			...
			// parallel code here
		 //i	unsigned char l_decrypted[1000], l_dcopy[1000];
			...
			int j = 0;
			int check_amount = 3;
			unsigned long long iteration_end= 32;   //pow(2, sizeof(int)*8 );
			int block_sz = iteration_end / comm_sz;
			int start = block_sz * my_rank;
			MPI_Status  status;


                
                 //
	         printf( "Processor %d,  will try %lu keys, from %lu to %lu\n", my_rank, block_sz, start , block_sz +start );   
		for ( i = my_rank ; i < iteration_end  ; i+= comm_sz ) {
				printf("Trying key %u from processor %u \n ", i, my_rank);
					if (  i %check_amount )
					{
						  printf("iteration num: %u\n ", i);
						  MPI_Test(&request[comm_sz] , &flag , MPI_STATUS_IGNORE ) ;
						 if( flag  )
						 {
							MPI_Recv(&flag, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 					
							printf("done : %d\n", my_rank );
...
//    printf("i=%d - decrypted: %s\n", i, decrypted); 
		    ...
		    if (isValid(dcopy, len)) 
			{
					...
					for (; j < comm_sz; ++j )
					{
					  MPI_Isend(&success, 1, MPI_INT, j, 0, MPI_COMM_WORLD, &request[comm_sz] );
