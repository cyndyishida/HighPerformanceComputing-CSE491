...
//  printf("Status:\n");
  ...
  fclose(fin);
	...
	MPI_Bcast( &encrypted, 1000, MPI_CHAR, 0, MPI_COMM_WORLD);
        MPI_Bcast( &len , 1 , MPI_CHAR, 0 , MPI_COMM_WORLD);
		...
		//	MPI_Irecv(&data,1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request[my_rank] );
		  ...
		  for ( i = my_rank  ; i <  iteration_end  ; i+= comm_sz) {
				...
				if ( (++counter) %check_amount )
				{
					//	printf("printed here at %u  ", counter );
						MPI_Test(&request[my_rank ],&flag ,MPI_STATUS_IGNORE);
					//	printf("passed test with flag: %u \n ", flag);
						if(flag)
						{
								printf( " flag: %u , rank: %u \n", flag, my_rank  );
      ...
      memcpy(dcopy, decrypted, len * sizeof(unsigned char));
			...
			for (; j < comm_sz; ++j )
			{
				...
				//MPI_Send(&success, 1, MPI_INT, j, 0, MPI_COMM_WORLD);
		      		if (j != my_rank)
				{
					MPI_Isend(&success, 1, MPI_INT, j, 0, MPI_COMM_WORLD, &request[my_rank]);
			//			printf( "Processor %u found the key, sending to: %u\n",my_rank, j );
				}
			}
			break;
		    }
